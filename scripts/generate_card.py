...
Tailored for Cristian Camilo Ramírez (cristiancamiloramireztorres89-sys).
"""
...
LOGIN = os.environ.get("GITHUB_LOGIN", "cristiancamiloramireztorres89-sys")
...
def ascii_rows_from_image(path):
    try:
        ...
        mask_img = Image.fromarray(alpha.astype(np.uint8), mode="L").resize((ART_COLS, rows), Image.Resampling.LANCZOS)
        gray_img = Image.fromarray(rgb.astype(np.uint8)).convert("L").resize((ART_COLS, rows), Image.Resampling.LANCZOS)
        ...
        print(f"warning: failed to process avatar image: {e}, falling back to dragon art", file=sys.stderr)
        return ascii_rows_dragon()
...
def fetch_github_stats(login, token):
    ...
    try:
        ...
        user_resp = requests.get(f"https://api.github.com/users/{login}", headers=headers, timeout=10)
        ...
        if repos_resp.status_code == 200:
            ...
            stats["stars"] = str(sum(r.get("stargazers_count", 0) for r in repos if isinstance(r, dict) and not r.get("fork")))
            lang_count = {}
            for r in repos:
...
def main():
    ...
    if avatar_path:
        ...
        print("no avatar found, using procedural dragon art")
